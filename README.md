...
run the one-shot command with a private password file whose exact bytes meet the
password policy:
...
non-superuser `NOBYPASSRLS` login roles and `workouts_security_owner` exists as a
non-login, non-bypass function owner. Job status, attempts, leases,
...
namespace `mailpit`, ephemeral PostgreSQL in namespace `postgresql`, and the
application in namespace `workouts-explorer`. It also creates development-only
rate-limit and bootstrap Secrets and bootstraps an `xdev-admin` identity so
invite and recovery mail can be inspected without external delivery. Context
`xtest` targets the separate test vCluster and can be selected with
`VCLUSTER_CONTEXT=xtest`. The database uses `emptyDir` and trust authentication
strictly for isolated vCluster testing. Application database URLs use
`postgres.postgresql.svc.cluster.local`, and development SMTP uses
...
enabled for vCluster sync-to-host. In the host namespace, vCluster rewrites the
Ingress reference to
`workouts-explorer-ingress-tls-x-<namespace>-x-<vcluster>` but does not rewrite a
synced Certificate's `spec.secretName`; the xdev harness overrides only
